...
class Keras_CNN():
    ...
    def predict(self, feature_converter):
        ...
        score = self.model.evaluate(x, z)
        # print self.model.predict_classes(x)
        # print self.label_dict
        # print score

        # labs = self.model.predict(x)
        # count = 0
        # correct = 0
        # for row in labs:
        #     max_val = None
        ...
        #         if max_index is None or row[i] > max_val:
        #             max_val = row[i]
        #             max_index = i

        #     if self.labels[max_index] == self.labels[y[count]]:
        #         correct += 1
        #     count += 1

        # # # print labs
        # print "Testing Accuracy: ", (correct / float(count) * 100), "%"
        return labels
